...
static int16_t last_alt=0;
static uint32_t last_snd_time=0;
static int16_t tmp_climb_rate=0;
 void NOINLINE Copter::send_vfr_hud(mavlink_channel_t chan)
 {
    tmp_climb_rate=(int16_t)((current_loc.alt-last_alt)/(((float)(AP_HAL::millis()-last_snd_time))/1000));
    mavlink_msg_vfr_hud_send(
      ...
      tmp_climb_rate / 100.0f
    );

    last_snd_time=AP_HAL::millis();
